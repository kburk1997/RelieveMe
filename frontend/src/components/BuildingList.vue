<template>
  <div class="columns">
    <div class="column is-2" id="filter-sidebar">
      ...
      >
        <b-checkbox
          @input="calculateFilters()"
          v-model="filters.selectedRegions"
          :native-value="region.regionId"
        >{{ region.name }}</b-checkbox>
      </div>
      <br>

      <b-field id="accessibility-filter" class="left" label="Filter by restroom features:"></b-field>
      <div
        class="field left"
      ...
      >
        <b-checkbox
          @input="calculateFilters()"
          v-model="filters.selectedAccessibilities"
          :native-value="accessibility.id"
        >{{ accessibility.name }}</b-checkbox>
      </div>
    </div>
      ...
      >
        ...
        <template slot-scope="props">
          <b-table-column field="properName" label="Name" sortable>
            <router-link
              :to="{name: 'Building', params:{name: props.row.properName}}"
            >{{props.row.properName}}</router-link>
          </b-table-column>
          <b-table-column field="region.name" label="Region" sortable>{{props.row.region.name}}</b-table-column>
        </template>
      </b-table>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    filterBuildingsByRestroomAccessibility: function (building) {
      ...
      for (
        var filterInd = 0;
        filterInd < this.filters.selectedAccessibilities.length;
        filterInd++
      ) {
        if (
          !this.buildingContainsAccessibility(
            building,
    ...
    buildingContainsAccessibility: function (building, accessibility) {
      ...
      for (var floorInd = 0; floorInd < building.floors.length; floorInd++) {
        if (
          this.floorContainsAccessibility(
            building.floors[floorInd],
            accessibility
          )
        ) {
          return true
    ...
    floorContainsAccessibility: function (floor, accessibility) {
      for (
        ...
        if (
          this.bathroomHasAccessibility(
            floor.bathrooms[bathroomInd],
            accessibility
          )
        ) {
          return true
